...
# This version includes automatic installation of Node.js and Ollama
...
APPLESCRIPT
                        ...
                        echo "=== AIPrivateSearch Uninstall Starting at $(date) ==="
                        ...
                        echo "=== Uninstall Complete at $(date) ==="
                        
                        osascript -e 'display dialog "AIPrivateSearch has been uninstalled successfully.\n\nRestart Terminal to complete the process." buttons {"OK"} with icon note'
...
if [ -d "repo/aiprivatesearch" ]; then
    ...
    if curl -L -o "\$REPO_ZIP" "\$REPO_ZIP_URL"; then
        ...
        echo "❌ CRITICAL ERROR: Repository download failed"
        echo "❌ Cannot continue without repository"
